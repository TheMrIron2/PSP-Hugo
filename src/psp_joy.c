...
psp_joy_default_settings()
{
  ...
  HUGO.psp_irdajoy_type       = 0;
  HUGO.psp_irdajoy_debug      = 0;
...
psp_joy_load_settings_file(FILE *JoyFile)
{
  ...
  while (fgets(Buffer,512,JoyFile) != (char *)0) {
    ...
    *Scan = '\0';
    ...
    if (!strcasecmp(Buffer,"psp_irdajoy_type")) HUGO.psp_irdajoy_type = Value;
    else
    if (!strcasecmp(Buffer,"psp_irdajoy_debug")) HUGO.psp_irdajoy_debug = Value;
    else
